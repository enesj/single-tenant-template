...
### Backend (Clojure)
```bash
clj-nrepl-eval -p <port> "(require '[some.ns :as ns] :reload) (ns/fn ...)"
```

### Frontend (ClojureScript)
Preferred: evaluate `.cljs` with the `mcp_clojure-mcp_clojurescript_eval` MCP tool.

If you must use `clj-nrepl-eval`, it starts in Clojure mode and cannot evaluate CLJS forms until you switch the session to a ClojureScript REPL:
```bash
clj-nrepl-eval -p 8777 "(require '[shadow.cljs.devtools.api :as shadow]) (shadow/repl :app)"   # or :admin
```
Exit CLJS mode with `:cljs/exit`.
