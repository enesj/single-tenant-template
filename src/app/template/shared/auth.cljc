...
#_(def role-owner "owner")
(def role-admin "admin")
(def role-member "member")
#_(def role-viewer "viewer")
#_(def role-unassigned "unassigned")

;; Core roles (domain-specific roles added separately)
#_(def core-roles #{role-owner role-admin role-member role-viewer role-unassigned})

;; Core role hierarchy (higher index = more permissions)
#_(def core-role-hierarchy [role-unassigned role-viewer role-member role-admin role-owner])

;; -------------------------
;; User Status Constants
;; -------------------------

#_(def status-active "active")
#_(def status-inactive "inactive")
#_(def status-pending "pending")
#_(def status-suspended "suspended")

#_(def valid-user-statuses #{status-active status-inactive status-pending status-suspended})

;; -------------------------
;; Authentication Providers
;; -------------------------

#_(def provider-google :google)
#_(def provider-github :github)

#_(def valid-providers #{provider-google provider-github})

;; -------------------------
;; Session Constants
;; -------------------------

#_(def session-timeout-hours 24)
#_(def session-timeout-ms (* session-timeout-hours 60 60 1000))
...
#_(defn is-admin-or-higher?
    "Check if user is admin or higher"
    [user role-hierarchy]
    (role-at-least? user role-admin role-hierarchy))

#_(defn is-member-or-higher?
