...
(defn create-webserver [host port database service-container]
  ...
  (let [routes (-> (routes/app-routes database service-container)
                 (wrap-service-container service-container))
        stop-server (http-kit/run-server routes
                      {:port port
                       :host host
                                           ...
                                           (if ex
                                             (do
                                               ...
                                               (when (.getCause ex)
                                                 ...
                                                 (doseq [element (take 10 (.getStackTrace ex))]
                                                   (log/error "  at " (str element)))))
                                             (log/error (str text))))))})
        server-instance (-> stop-server meta :server)]
    ...
    (reify
      ...
      (close [_]
        (if server-instance
          (do
            ;; Wait until the http-kit server thread actually completes so a dev
            ;; restart can bind the same port immediately.
            (when-let [stopped (http-kit/server-stop! server-instance {:timeout 1000})]
              @stopped)
            (log/info "Server stopped"))
          (do
            (stop-server :timeout 1000)
            (log/info "Server stop signalled")))))))
