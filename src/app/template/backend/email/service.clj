       ...
       (let [postmark-api-key (-> config :postmark :api-key)
         ...
         (if (and postmark-api-key from-email)
           (do
             ...
             (postmark-email/create-postmark-email-service postmark-api-key from-email base-url db))
           (do
       ...
       (let [smtp-config (get config :smtp)
         ...
         (if (and smtp-config from-email)
           (do
             ...
             (gmail-smtp/create-gmail-smtp-email-service smtp-config from-email base-url db))
           (do
       ...
       (let [api-config (get config :gmail-api)
         ...
         (if (and api-config from-email
           ...
           (do
             ...
             (gmail-api/create-gmail-api-email-service api-config from-email base-url db))
           (do
