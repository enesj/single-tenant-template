...
(defn login-handler
  ...
  (utils/with-error-handling
    (fn [request]
      (let [body (or (:body request) (:body-params request) {})
            email (some-> (or (:email body) (get body "email")) str str/trim not-empty)
            password (some-> (or (:password body) (get body "password")) str not-empty)
