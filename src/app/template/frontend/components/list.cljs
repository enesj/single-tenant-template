(ns app.template.frontend.components.list
  (:require
    ...
    [app.template.frontend.components.list.gates :as list-gates]
    [app.template.frontend.components.list.handlers :as list-handlers]
    [app.template.frontend.components.list.render :as list-render]
    [app.template.frontend.components.list.rows :refer [render-row]]
    [app.template.frontend.components.list.rows-override :as list-rows-override]
    [app.template.frontend.components.list.table :refer [make-table-headers]]
    [app.template.frontend.components.list.toggles :as list-toggles]
...
(defn- toggle-group-pill
  [group]
  (list-toggles/toggle-group-pill group))

(defn- apply-rows-override-transforms
  [opts]
  (list-rows-override/apply-rows-override-transforms opts))

(defn- selected-item?
  [selected-ids item]
  (list-rows-override/selected-item? selected-ids item))

(defn- apply-selection-visibility
  [rows selected-ids display-settings]
  (list-rows-override/apply-selection-visibility rows selected-ids display-settings))

(defn- gate-allows-action?
  [gate-id gate-ctx]
  (list-gates/gate-allows-action? gate-id gate-ctx))
...
(defui list-view
  ...
  (let [entity-kw (if (keyword? entity-name) entity-name (keyword entity-name))
    ...
    (let [{:keys [handle-select-change
      ...
      (list-render/render-list-view-shell
        {:entity-name entity-name
         :entity-kw entity-kw
         :entity-spec entity-spec
         :props props
         ...
         :resolved-add-modal-title resolved-add-modal-title
         :form-entity-spec form-entity-spec
         :has-custom-add-form? has-custom-add-form?
         ...
         :resolved-edit-modal-title resolved-edit-modal-title
         :form-entity-spec-edit form-entity-spec-edit
         :has-custom-edit-form? has-custom-edit-form?
         :render-edit-form render-edit-form
         :handle-edit-modal-close handle-edit-modal-close
         :handle-edit-modal-success handle-edit-modal-success
         :batch-edit-inline-state batch-edit-inline-state
         :selected-ids selected-ids
         :error error
         :loading? loading?
         :raw-items raw-items
         :show-inline-add-form? show-inline-add-form?
         :base-props base-props
         :visible-items visible-items
         :effective-disallowed-action-mode effective-disallowed-action-mode
         :merged-display-settings merged-display-settings
         :effective-allow-add? effective-allow-add?
         :table-width table-width
         :title title
         :show-add-form? show-add-form?
         :use-modal-forms? use-modal-forms?
         :handle-add-click handle-add-click
         :active-filters active-filters
         :active-inline-filter active-inline-filter
         :inline-filter-field-spec inline-filter-field-spec
         :inline-filter-value inline-filter-value
         :handle-filter-close handle-filter-close
         :handle-filter-apply handle-filter-apply
         :set-active-inline-filter set-active-inline-filter
         :set-inline-filter-field-spec set-inline-filter-field-spec
         :set-inline-filter-value set-inline-filter-value
         :sorts sorts
         :sort-field-labels sort-field-labels
         :ui-state ui-state
         :hardcoded-view-options hardcoded-view-options
         :selected-count selected-count
         :hidden-selected-count hidden-selected-count
         :toggle-group-pill toggle-group-pill
         :shell-ref shell-ref
         :table-height table-height
         :table-headers table-headers
         :show-highlights? show-highlights?
         :render-row-fn render-row-fn
         :measured-table-height measured-table-height
         :measured-table-width measured-table-width
         :effective-per-page effective-per-page
         :on-per-page-change on-per-page-change
         :pagination-total-pages pagination-total-pages
