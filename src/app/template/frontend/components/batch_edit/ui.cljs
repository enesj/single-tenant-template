...
(defui batch-edit-header
  ...
  [{:keys [selected-ids selected-count on-close]}]
  (let [t (i18n/use-t)
        item-count (or selected-count (count selected-ids) 0)]
    ($ :div {:class "flex items-center justify-between mb-4"}
      ($ :div {:class "flex items-center"}
        ...
        ($ :h4 {:class "text-md font-semibold text-primary"}
          (str (t :list/toggle-batch-edit) " (" item-count ")")))
      ...
      ($ button
        {:btn-type :save
         ...
         :disabled (boolean (or submitting?
                              has-form-errors?
                              no-changes?))
         :children [(t :common/update-all) ($ save-icon)]}))))
    ...
    ($ :div
      ...
      (when (and form-success? submitted?)
        ($ messages/success-alert
          {:message (t :common/batch-update-success)
           :entity-name entity-name})))))
