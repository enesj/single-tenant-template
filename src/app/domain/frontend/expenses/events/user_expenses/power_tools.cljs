      ...
      {:db (begin-entity-load db :manufacturers)
       :http-xhrio (x/xhrio db
                     {:method :get
                      ...
                      :admin-uri endpoints/admin-manufacturers-endpoint
                      :params request-params
...
(rf/reg-event-fx
  ...
  common-interceptors
  (fn [{:keys [db]} [response]]
    (let [rows (vec (or (:manufacturers response) (:data response) []))]
      {:db (finish-entity-load db :manufacturers nil)
       :dispatch [::expenses-sync/sync-manufacturers rows]})))
...
(rf/reg-event-fx
  ...
  common-interceptors
  (fn [{:keys [db]} [{:keys [display_name display-name]} on-success]]
    (let [display-name* (or display_name display-name)]
      {:db (-> db
             (assoc-in [:user-expenses :form :loading?] true)
             (assoc-in [:user-expenses :form :error] nil))
       :http-xhrio (x/xhrio db
...
(rf/reg-event-fx
  ...
  common-interceptors
  (fn [{:keys [db]} [on-success response]]
    (let [manufacturer (or (:manufacturer response) (:data response) response)]
      {:db (-> db
             (assoc-in [:user-expenses :form :loading?] false)
             (assoc-in [:user-expenses :form :error] nil))
      ...
      {:db (-> db
       ...
       :http-xhrio (x/xhrio db
                     {:method :put
                      ...
                      :admin-uri (str endpoints/admin-manufacturers-endpoint "/" manufacturer-id-str)
                      :params (or form-data {})
...
(rf/reg-event-fx
  ...
  (fn [{:keys [db]} [manufacturer-id on-success _response]]
    {:db (-> db
           (assoc-in [:user-expenses :form :loading?] false)
           (assoc-in [:user-expenses :form :error] nil)
           (cond-> (seq (some-> manufacturer-id str))
             (crud-success/track-recently-updated :manufacturers (str manufacturer-id))))
     :dispatch-n [[:user-expenses/fetch-manufacturers]]
     :fx [(when on-success
            [:dispatch-later {:ms 100
                              :dispatch [:user-expenses/call-modal-callback on-success]}])]}))
...
;; ---------------------------------------------------------------------------
;; Manufacturer aliases (power-user list + basic edit/delete)
;; ---------------------------------------------------------------------------

(rf/reg-event-fx
  :user-expenses/fetch-manufacturer-aliases
  common-interceptors
  (fn [{:keys [db]} [params]]
    (let [request-params (merge {:limit 200 :offset 0} (when (map? params) params))]
      {:db (begin-entity-load db :manufacturer-aliases)
       :http-xhrio (x/xhrio db
                     {:method :get
                      :uri endpoints/manufacturer-aliases-endpoint
                      :admin-uri endpoints/admin-manufacturer-aliases-endpoint
                      :params request-params
                      :on-success [:user-expenses/fetch-manufacturer-aliases-success]
                      :on-failure [:user-expenses/fetch-manufacturer-aliases-failure]})})))

(rf/reg-event-fx
  :user-expenses/fetch-manufacturer-aliases-success
  common-interceptors
  (fn [{:keys [db]} [response]]
    (let [aliases (vec (or (:data response) (:manufacturer-aliases response) []))]
      {:db (finish-entity-load db :manufacturer-aliases nil)
       :dispatch [::expenses-sync/sync-manufacturer-aliases aliases]})))

(rf/reg-event-db
  :user-expenses/fetch-manufacturer-aliases-failure
  common-interceptors
  (fn [db [error]]
    (finish-entity-load db :manufacturer-aliases error)))

(rf/reg-event-fx
  :user-expenses/update-manufacturer-alias-modal
  common-interceptors
  (fn [{:keys [db]} [alias-id form-data on-success]]
    (let [alias-id-str (some-> alias-id str)
          raw-label (or (:raw_label form-data) (:raw-label form-data))
          raw-label-normalized (or (:raw_label_normalized form-data) (:raw-label-normalized form-data))
          payload (cond-> {}
                    (some? raw-label) (assoc :raw_label raw-label)
      ...
      {:db (-> db
       ...
       :http-xhrio (x/xhrio db
                     {:method :put
                      :uri (str endpoints/manufacturer-aliases-endpoint "/" alias-id-str)
                      :admin-uri (str endpoints/admin-manufacturer-aliases-endpoint "/" alias-id-str)
                      :params payload*
                      :on-success [:user-expenses/update-manufacturer-alias-modal-success alias-id-str on-success]
                      :on-failure [:user-expenses/update-manufacturer-alias-modal-failure]})})))

(rf/reg-event-fx
  :user-expenses/update-manufacturer-alias-modal-success
  common-interceptors
  (fn [{:keys [db]} [alias-id on-success _response]]
    (let [highlight-id (some-> alias-id str)]
      {:db (-> db
             (assoc-in [:user-expenses :form :loading?] false)
             (assoc-in [:user-expenses :form :error] nil)
             (cond-> highlight-id
               (crud-success/track-recently-updated :manufacturer-aliases highlight-id)))
       :dispatch-n [[:user-expenses/fetch-manufacturer-aliases]]
       :fx [(when on-success
              [:dispatch-later {:ms 100
                                :dispatch [:user-expenses/call-modal-callback on-success]}])]})))

(rf/reg-event-db
  :user-expenses/update-manufacturer-alias-modal-failure
  common-interceptors
  (fn [db [error]]
    (-> db
      (assoc-in [:user-expenses :form :loading?] false)
...
(rf/reg-event-fx
  :user-expenses/delete-manufacturer-alias
  common-interceptors
  (fn [{:keys [db]} [alias-id]]
    (let [alias-id-str (some-> alias-id str)]
      {:db (-> db
       ...
       :http-xhrio (x/xhrio db
                     {:method :delete
                      :uri (str endpoints/manufacturer-aliases-endpoint "/" alias-id-str)
                      :admin-uri (str endpoints/admin-manufacturer-aliases-endpoint "/" alias-id-str)
                      :on-success [:user-expenses/delete-manufacturer-alias-success]
                      :on-failure [:user-expenses/delete-manufacturer-alias-failure]})})))

(rf/reg-event-fx
  :user-expenses/delete-manufacturer-alias-success
  common-interceptors
  (fn [{:keys [db]} [_response]]
    {:db (assoc-in db [:user-expenses :form :loading?] false)
     :dispatch [:user-expenses/fetch-manufacturer-aliases]}))

(rf/reg-event-db
  :user-expenses/delete-manufacturer-alias-failure
  common-interceptors
  (fn [db [error]]
    (-> db
      (assoc-in [:user-expenses :form :loading?] false)
