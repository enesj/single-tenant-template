(ns app.domain.frontend.expenses.pages.user.manufacturers
  "Power-user manufacturers list (canonical brands) used to normalize article manufacturers."
  (:require
    [app.domain.frontend.expenses.authz :as authz]
    [app.domain.frontend.expenses.components.page-guard :refer [expenses-page-guard]]
    [app.domain.frontend.expenses.components.user-power-forms :refer [user-manufacturer-add-form-modal
                                                                      user-manufacturer-edit-form-modal]]
...
(defn- render-add-form
  ...
  ($ user-manufacturer-add-form-modal
...
(defn- render-actions
  ...
  (let [manufacturer-id (id-utils/extract-entity-id item)
    ...
    ($ :div {:class "flex items-center justify-center gap-2"}
      (when show-edit?
        ($ button
          {:id (str "btn-edit-manufacturers-" manufacturer-id-str)
           :btn-type :primary
           :shape "circle"
      ...
      (when show-delete?
        ($ button
          {:id (str "btn-delete-manufacturers-" manufacturer-id-str)
           :btn-type :danger
           :shape "circle"
...
(defui manufacturers-page []
  (let [role (use-subscribe [:expenses/user-role])
        ...
        refresh-list (use-callback
                       (fn []
                         (rf/dispatch [:user-expenses/fetch-manufacturers]))
                       [])]
    ...
    ($ expenses-page-guard
      {:capability :expenses/manufacturers.manage
       :children
             ...
             ($ :div {:class "flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4"}
               ($ :div
                 ...
                 ($ :p {:class "text-sm text-base-content/70"}
                   "Power-user manufacturer catalog (used for normalizing article manufacturers)"))
               ($ :div {:class "flex gap-2 flex-wrap"}
                 ($ button {:id "btn-back-expenses-dashboard-manufacturers"
                            :btn-type :ghost
                            :on-click #(rf/dispatch [:navigate-to "/expenses"])}
                   "Dashboard")
                 ($ button {:id "btn-go-manufacturer-aliases-manufacturers"
                            :btn-type :primary
                            :on-click #(rf/dispatch [:navigate-to "/manufacturer-aliases"])}
