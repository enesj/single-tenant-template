(ns app.domain.frontend.expenses.pages.admin.articles
  (:require
...
(defn- render-edit-form
  ...
  (let [article-id (id-utils/extract-entity-id item)
        initial-values (assoc item :id article-id)]
    ($ entity-form-modal
      {:entity-name entity-name
       :entity-spec entity-spec
       :editing? true
       :initial-values initial-values
       :button-text "Update Article"
       :on-success on-success
       :on-cancel on-cancel})))

(defui admin-articles-page []
  (let [refresh-list #(rf/dispatch [::articles-events/load-list {}])]
