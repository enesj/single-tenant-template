...
(defui user-supplier-select-with-inline-create
  ...
  (let [t (i18n/use-t)
        creating? (boolean (use-subscribe [:user-expenses/form-loading?]))
        ...
        submit (fn []
                 (let [name* (str/trim (str display-name))]
                   (cond
                     (str/blank? name*)
                     (set-local-error! (t :common/display-name-required))
    ...
    ($ :div {:class wrapper-class}
      ($ common/label {:text (or label (t :common/supplier))
                       :class label-class
                       :for select-id
                       :required required})
            ...
            ($ :div {:class "flex justify-end gap-2 pt-4"}
              ...
              ($ :button {:id save-btn-id
                          ...
                          :on-click (fn [e]
                                      ...
                                      (.stopPropagation e)
                                      (submit))}
                (if creating? (t :common/saving) (t :common/create))))))))))
