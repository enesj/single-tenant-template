  ...
  (let [current (use-subscribe [::dup-events/strategy])]
    ($ :div {:class "ds-join mb-4" :id "dedup-strategy-selector"}
      (for [{:keys [key label]} strategies]
        ($ :button {:key key
                    :id (str "dedup-strategy-" key)
                    :class (str "ds-btn ds-join-item ds-btn-sm"
                             (if (= current key) " ds-btn-primary" " ds-btn-outline"))
                    :on-click #(rf/dispatch [::dup-events/set-strategy key])}
          label)))))
...
(defui admin-duplicates-page []
  (let [loading? (use-subscribe [::dup-events/loading?])
        error (use-subscribe [::dup-events/error])
    ...
    ($ layout/admin-layout
      ($ :div {:class "p-6 min-h-screen"}
        ...
        ;; Strategy selector + detect button
        ($ :div {:class "flex items-center gap-4 mb-6"}
          ($ strategy-selector)
          ($ :button {:id "dedup-detect-btn"
                      :class (str "ds-btn ds-btn-primary ds-btn-sm"
                               (when loading? " ds-loading"))
                      :disabled loading?
                      :on-click #(rf/dispatch [::dup-events/detect
                                               {:entity-type entity-type
                                                :strategy strategy}])}
            "Detect Duplicates"))
        ...
        (cond
          ...
          ($ :div {:class "ds-alert ds-alert-info" :id "dedup-no-results"}
            ($ :span "No duplicates found with current settings. Try a different strategy or lower the threshold."))
