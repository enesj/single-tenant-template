...
2. Add new email storage columns through migrations.
...
3. Backfill existing data and enable dual-write.
...
4. Replace exact-match lookups with blind-index lookups.
...
5. Rework invitation flows to use exact email identity without plaintext lookup.
...
6. Convert routine admin surfaces to pseudonymous projections.
...
7. Add audited exceptional identity-reveal paths.
...
8. Add generic payment-provider account mappings.
...
9. Remove raw email from session blobs, audit logs, and operational logs where it is not required.
...
10. Remove legacy plaintext email columns after a stable transition window.

Goal: complete the migration and ensure email is no longer stored in plaintext in the main tables.

Files:
- `/Users/enes/Projects/single-tenant-template/resources/db/migrations/<drop-plaintext-email-migration>.edn`
- `/Users/enes/Projects/single-tenant-template/resources/db/models.edn`
...
Notes:
- Only drop plaintext columns after backfill verification and dual-read burn-in.
- Ensure all read paths depend only on ciphertext and lookup hash before removal.
...
- Assumption: normalized email comparison uses a lower-cased canonical form.
- Assumption: partial substring email search is removed from admin list contracts.
- Assumption: routine admin surfaces do not display full or masked user email.
- Assumption: invitation mismatch errors are allowed to show the exact invited email.
- Assumption: billing integrations will use a generic payment-provider abstraction rather than a Stripe-specific model.
- Open question: whether nickname fully replaces `full_name` in the product or whether both remain temporarily during rollout.
- Open question: whether key rotation is implemented immediately or introduced after the first stable release of encrypted-at-rest email.
