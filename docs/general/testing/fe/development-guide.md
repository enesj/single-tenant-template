...
**Solution**: Ensure `app.template.frontend.dev.repl-tracing` is included in the `:app` dev preloads (`shadow-cljs.edn` → `:builds :app :dev :modules :app :preloads`), then reload the page so your CLJS REPL session (e.g. via `clojure-mcp` eval tools connected to the shadow nREPL) can `require` it.
