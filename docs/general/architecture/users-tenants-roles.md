...
This app has no tenants. Two principal actor types exist: **admins** (platform operators) and **users** (end users). Admin actions and logins are monitored via `audit_logs`/`login_events`.
...
- **audit_logs**: Records admin/user actions. Fields: actor_id, actor_type (`admin|user`), action, metadata, created_at, updated_at.
- **login_events**: Records login attempts. Fields: principal_id, principal_type, success, reason, ip, user_agent, created_at, updated_at.
...
- `updated_at` is maintained by PostgreSQL triggers via `update_updated_at_column()` and per-table `<table>_updated_at` triggers defined in `resources/db/template/triggers.edn`.
