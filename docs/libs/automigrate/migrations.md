...
| `:check`       | Set condition in HoneySQL format to create custom CHECK for a column. For operators or expressions that HoneySQL doesn't model as keywords (such as PostgreSQL regex match `~*`), wrap the full SQL expression in a `[:raw "..."]` vector. | `false`   | Examples: `[:and [:> :month 0] [:<= :month 12]]`, `[:like :email "%@%.%"]`, or `[:raw "email ~* '^[^\\s@]+@[^\\s@]+\\.[^\\s@]+$'"]` |
