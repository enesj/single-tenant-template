...
7. **Reports**: Interactive analytics widgets (supplier deep-dive, day-of-week, top items, monthly comparison, size distribution, daily heatmap, category allocation) (`expense_reports.cljs`). Page-level dropdown filters are sent as shared report-scope API params (range/supplier/category/subcategory/expense-category/manufacturer), while click interactions inside widgets remain local UI filters until cleared.
