    ...
    (let [[env backup-file] args]
      (if (#{"dev" "test"} env)
        ...
        (do
          ...
          (println "Usage: clj -M scripts/bb/database/restore_db_legacy.clj [dev|test] backup_file.sql")
          (System/exit 1))))
    (do
      (println "Usage: clj -M scripts/bb/database/restore_db_legacy.clj [dev|test] backup_file.sql")
      ...
      (println)
      (println "Examples:")
      (println "  clj -M scripts/bb/database/restore_db_legacy.clj dev backups/backup_dev_2025-06-27_19-58-16.sql")
      (println "  clj -M scripts/bb/database/restore_db_legacy.clj test backups/backup_dev_2025-06-27_19-58-16.sql")
      (println)
      (println "Available backup files:")
      (let [backups-dir (java.io.File. "backups")]
        (if (.exists backups-dir)
