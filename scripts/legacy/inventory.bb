...
(def categories
  {
   ...
   :legacy-route-redirects  ["legacy.*route|redirect.*legacy"]
   :localstorage-migration  ["localStorage|LocalStorage|local-storage.*migrat"]
   :settings-schema-legacy  ["legacy.*settings|settings.*fallback"]
...
(defn run-file-glob [category globs]
  (let [{:keys [exit out]} (apply rg* "--files" (concat ["src" "test"] (map #(str "-g" "=" %) globs)))
    ...
    (map (fn [f] {:category category :file f :line 0 :match "<file>" :pattern (str (str/join "," globs))}) files)))
...
(defn -main [& args]
  (let [{:keys [out]} (parse-args (vec args))
        inv (inventory)
        edn-str (with-out-str (prn inv))]
    (println edn-str)
    (when out
      (fs/create-dirs (fs/parent out))
      (spit out (str ";; generated by scripts/legacy/inventory.bb\n" edn-str)))))
