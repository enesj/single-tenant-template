...
ensure_shadow_cljs() {
    ...
    # Prefer port check (works even if pid file is missing/stale).
...
ensure_worktree_public_assets() {
    ...
    # If we previously symlinked the entire folder, replace it with a real directory.
    # (We only want to share stable assets like CSS/images, not JS build output.)
    ...
    # Copy HTML entrypoints if missing.
    ...
    # Share stable assets (CSS/images) from main to avoid duplication.
