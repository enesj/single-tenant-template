...
(deftest get-view-options-handler-test
  ...
  (testing "get-view-options returns empty map when no config exists"
    (let [db (h/mock-db)
          handler (settings/get-view-options-handler db)
          request (h/mock-admin-request :get "/admin/api/settings/view-options" mock-admin {})]
...
(deftest update-entity-setting-handler-test
    ...
    (let [db (h/mock-db)
          ...
          request (h/mock-admin-request :patch "/admin/api/settings/entity" mock-admin
                    {:body {:entity-name "users"
                            ...
                            :setting-value :email}})]
      (with-redefs [settings-io/read-view-options (fn [_db] mock-view-options)
                    settings-io/write-view-options! (fn [_db opts]
  ...
  (testing "update-entity-setting rejects underscore field names"
    (let [db (h/mock-db)
      ...
      (with-redefs [settings-io/read-view-options (fn [_db] mock-view-options)
                    settings-io/write-view-options! (fn [_db _opts] nil)]
        (let [response (handler request)]
          (is (= 400 (:status response)))))))
