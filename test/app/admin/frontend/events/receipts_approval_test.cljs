...
(deftest update-posted-receipt-uses-admin-expenses-update-posted-endpoint
  (testing ":admin/update-posted-receipt targets the admin expenses posted-update endpoint"
    (setup/reset-db!)
    (setup/install-http-stub!)

    (rf/dispatch-sync [:admin/update-posted-receipt "receipt-123" {:supplier_id "sup-1"} nil])

    (let [req (setup/last-http-request)]
      (is (= :post (:method req)))
      (is (= "/admin/api/expenses/receipts/receipt-123/update-posted" (:uri req)))
      (is (= {:supplier_id "sup-1"} (:params req)))
      (is (= "test-token" (get-in req [:headers "x-admin-token"])))
      (is (true? (get-in @rf-db/app-db [:admin :receipts :detail :action-loading?]))))))
