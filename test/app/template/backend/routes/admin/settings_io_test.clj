  ...
  (testing "admin defaults override domain defaults for the same entity"
    (let [dir (temp-dir)
      ...
      (clojure.core/with-redefs-fn
        {#'settings-io/view-options-path admin-path
        ...
        (fn []
          (is (= true
                (get-in (settings-io/read-view-options :mock-db)
                  [:receipts :display-locks :show-edit?]))))))))

(deftest read-view-options-runtime-override-wins
  (testing "runtime override wins over both admin and domain defaults"
    (let [dir (temp-dir)
          admin-path (tmp-path dir "admin-view-options.edn")
          domain-path (tmp-path dir "domain-view-options.edn")]
      (write-edn! admin-path {:receipts {:display-locks {:show-edit? true}}})
...
(deftest read-form-fields-admin-overrides-domain
  (testing "admin defaults override domain defaults for the same entity"
    (let [dir (temp-dir)
      ...
      (clojure.core/with-redefs-fn
        {#'settings-io/form-fields-path admin-path
        ...
        (fn []
          (is (= [:admin]
                (get-in (settings-io/read-form-fields :mock-db) [:receipts :create]))))))))
